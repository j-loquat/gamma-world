    ...
    try:
        ...
        for i, creature in enumerate(creatures):
            if not isinstance(creature, dict) or "name" not in creature:
                raise ValueError(
                    f"Invalid format in creature entry {i + 1}: missing 'name' or not an object."
                )
...
def generate_ai_image(creature_data: Dict[str, Any]) -> Tuple[str, Optional[bytes]]:
    ...
    if STYLE_IMAGE_PATH.exists():
        try:
            ...
            cprint(
                f"Warning: Could not load style image '{STYLE_IMAGE_PATH}': {e}. Proceeding without it.",
                "yellow",
            )
    else:
        cprint(
            f"Warning: Style image not found at {STYLE_IMAGE_PATH}. Proceeding without style reference.",
            "yellow",
        )
    ...
    try:
        ...
        if image_part:
            ...
            if not mime_type.lower().endswith("png"):
                try:
                    ...
                    cprint(
                        f"Warning: Failed to convert image to PNG: {e}. Using original format.",
                        "yellow",
                    )
        ...
        error_msg = (
            f"AI image generation failed due to an unexpected error: {type(e).__name__}: {e}"
        )
...
def prompt_user_generated(creature_name: str, temp_path: Path) -> str:
    ...
    while True:
        cprint(
            "Options: [O]pen Generated, [A]ccept and Save, [D]iscard and Skip, [R]egenerate, [Q]uit",
            "magenta",
            end="",
        )
        choice = input(" > ").upper()
...
def main_loop():
    ...
    try:  # Wrap main loop in try/finally for cleanup
        for i, creature in enumerate(creatures):
            ...
            cprint(
                f"\n--- Processing Creature {processed_count}/{total_creatures}: {name} ---",
                "blue",
                attrs=["bold"],
            )
            ...
            current_temp_image_bytes: Optional[bytes] = (
                None  # Track generated image data for the current loop
            )
            ...
            while action != "S":  # Loop until skipped or quit
                ...
                if action in ["G", "R"]:
                    if not client:
                        cprint(
                            "Cannot generate image: AI Client not initialized (check API key). Skipping.",
                            "red",
                        )
                    ...
                    if gen_status == "success" and img_data:
                        ...
                        error_msg = (
                            img_data.decode("utf-8") if img_data else "Unknown generation error."
                        )
...
if __name__ == "__main__":
    if not client and (not GEMINI_API_KEY or GEMINI_API_KEY == "YOUR_GEMINI_API_KEY_HERE"):
        cprint(
            "Exiting: AI Client could not be initialized. Please provide API Key.",
            "red",
            attrs=["bold"],
        )
    else:
