            ...
            with config.INDEX_FILE.open("r", encoding="utf-8") as f:
                try:
                    ...
                    if not isinstance(idx, list):
                        log.warning(
                            f"Index file {config.INDEX_FILE} is corrupted (not a list). Overwriting."
                        )
                        idx = []
                except json.JSONDecodeError:
                    log.warning(
                        f"Index file {config.INDEX_FILE} is corrupted (invalid JSON). Overwriting."
                    )
                    idx = []
        idx.append(rec)
...
def load_mutations(filepath: Path) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        log.info(
            f"Successfully loaded {len(mutations)} mutations from {filepath} under key '{key}'."
        )
        return mutations
